...
| ------------------------ | -------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------- | ----------------------------------------------------- |
| **Hardik**       | Integrator: merges, contracts, API server, live data, demo hardening | `shared/`, `server/` core + `routes/` + `sources/` + `store.ts`, `src/App.tsx`, `src/routes.ts`, `src/api/`, configs, `package*.json`, docs, `tasks.json` | Everyone's PRs                                        | Everyone (contract updates, merged main)              |
| **Affan**        | AI: real provider, explanations, Ask ORBIT                           | `server/ai/`, `server/routes/ai.ts`, `src/features/insight/`, `src/features/ask/`                                                                         | Hardik (contract changes), Arham (primitives, layout) | Ayman/Arham (InsightCard), Hardik (AI cache for demo) |
| **Arham** | Design system, app shell, cinematic globe, global view               | `src/styles/`, `src/ui/`, `src/globe/`, `src/layout/`, `src/state/`, `public/`                                                                            | Shrey (data density), Hardik                          | Everyone (primitives)                                 |
| **Ayman**       | Country, event and timeline views                                    | `src/features/country/`, `src/features/event/`, `src/features/timeline/`                                                                                  | Arham (primitives), Affan (InsightCard), Shrey (data) | Arham (layout fit)                                    |
| **Shrey**        | Curated data, news and health panels, demo script                    | `server/data/seed/`, `src/features/news/`, `src/features/health/`, `docs/DEMO.md`                                                                         | Hardik (contracts)                                    | Everyone (data), Hardik (demo script)                 |
