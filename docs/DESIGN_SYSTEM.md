...
**Direction:** deep black, electric cyan, amber/orange accents. Futuristic, cinematic, clean, like an
...
`--shadow-panel` (panel depth), `--glow-primary` (cyan), `--glow-accent` (amber), `--glow-alert` (orange), `--blur-panel`
(glass blur). Use glows sparingly: for focus, hover and severity 5 only.

### Motion and layers

- `--duration-fast/base/slow` (150/250/600ms) with `--ease-out`. They become 0 when the user prefers reduced motion.
- `--z-globe` < `--z-overlay` < `--z-panel` < `--z-modal`.
- Layout sizes: `--panel-width` (420px), `--topbar-height` (56px).

## Primitives (`src/ui`, import from `../../ui`)

| Component       | Use                                                                                                                     |
| --------------- | ----------------------------------------------------------------------------------------------------------------------- |
| `Panel`         | Every overlay section. Props: `eyebrow`, `title`, `actions`, `children`                                                 |
...
| `SeverityBadge` | Shows severity 1–5 with its colour and label                                                                            |

Button and Tag are planned (task ORB-ARH-01). Ask Arham before creating a new primitive.
